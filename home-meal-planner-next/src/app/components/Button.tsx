...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = "primary", className = "", disabled, icon, showTextOnMobile = false, children, ...props }, ref) => {
    const { theme } = useTheme();
    const themeClass = themeStyles[theme]?.[variant] || themeStyles["white"][variant];
    return (
      <button
        ref={ref}
        className={`px-3 py-1 rounded font-semibold transition text-sm sm:text-base shadow ${themeClass} ${disabled ? "opacity-50 cursor-not-allowed" : ""} ${className}`}
        disabled={disabled}
        {...props}
      >
        {icon ? (
